...
| Decisions to lock | Lock ICP, scored entity, horizon, capacity, region, data sensitivity, and pricing before model work. | `docs/architecture/ADRs/ADR-001-mvp-architecture.md` locks layout and stores; product defaults still open | pending |
...
| Repository and deployment shape | Intended layout is `apps/platform-service`, `apps/worker`, `apps/console`, contracts, docs, and tests. | ADR-001 accepted; `apps/` not created yet | pending |
