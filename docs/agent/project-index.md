...
| External API and data contracts | Versioned OpenAPI first. Account upsert, batch events, churn definition, predictions, webhooks, usage. | `contracts/openapi/churn-api.yaml` defines byte-bounded events:batch plus cursor-paginated current and historical prediction reads; account upsert remains next | pending |
...
| Milestone 3 — contract-first ingestion | OpenAPI plus idempotent `POST /v1/events:batch`. | OpenAPI + idempotent tenant-scoped PostgreSQL ingest are implemented; account upsert remains later | pending |
