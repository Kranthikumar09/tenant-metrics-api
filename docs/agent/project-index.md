...
| Tenant isolation and security | Strip client tenant headers; resolve tenant from a verified credential; tenant-scope every query and side effect. | header strip, hashed API-key TenantContext, tenant-scoped event writes, and accepted browser-session ADR exist; no session or OIDC code yet | pending |
...
| SaaS control plane | Onboarding, RBAC, entitlements, usage, billing, console, supportability. | console shell and accepted browser-session contract exist; session, RBAC, entitlements, usage, and billing are later | pending |
...
| Milestone 2 — identity and tenancy | OIDC, API keys, immutable TenantContext, RBAC, audit, tenant-scoped schema. | hashed API keys and TenantContext exist; ADR-002 defines OIDC/session boundary; browser session, RBAC, and audit code are later | pending |
...
| Milestone 6 — prediction API and console | Tenant-safe prediction reads and Angular onboarding/risk console. | prediction GET endpoints, cursor pagination, console shell, and browser-session ADR exist; console does not authenticate or fetch scores yet | pending |
