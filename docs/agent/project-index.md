...
| External API and data contracts | Versioned OpenAPI first. Account upsert, batch events, churn definition, predictions, webhooks, usage. | `contracts/openapi/churn-api.yaml` defines events:batch plus cursor-paginated current and historical prediction reads | pending |
...
| Milestone 6 — prediction API and console | Tenant-safe prediction reads and Angular onboarding/risk console. | current/history prediction GET endpoints, cursor pagination, session/membership foundation, and current-risk console reads exist; login UI and history visualization are later | pending |
