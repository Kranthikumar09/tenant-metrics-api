...
- Branch: `cursor/pr-035r-absolute-session-lifetime-9d98`
...
| Tests | platform-service context, health, PostgreSQL bootstrap, browser-session cookie/persistence/CSRF/absolute-expiry, OIDC Authorization Code/state/nonce/PKCE contract, OIDC membership mapping and failure, membership resolution and deny-by-default routes, tenant-isolation, event-batch, persistence, transactional-outbox enqueue, rules-score, prediction-read, prediction-cursor, and immutable tenant-history tests; shared rules-scoring unit tests; worker context-load, consume, bounded DLQ redrive, successful-delete, and rescore tests; console onboarding/current-risk/history contract tests |
