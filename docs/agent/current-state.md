...
Last updated for PR-041R — implement tenant-scoped account upsert.

## Snapshot

`/apps/platform-service` is a Java 21 Spring Boot 4.1.1 modular monolith with JDBC, Flyway, hashed-key TenantContext, PostgreSQL-backed Spring sessions, a 30-minute idle timeout, a configurable eight-hour default absolute session lifetime, CSRF-protected POST browser logout, request-time active-membership revocation, browser CSRF protection, a provider-neutral OIDC Authorization Code login adapter with S256 PKCE, server-side tenant membership resolution, tenant-scoped account upsert and event persistence, a PostgreSQL transactional outbox for LocalStack SQS delivery, `RULES_BASELINE` scores, and cursor-paginated prediction reads. `POST /v1/accounts:upsert` now atomically creates a tenant-owned account with 201 or returns its same-tenant retry with 200; the composite PostgreSQL key supplies concurrency-safe idempotency. `POST /v1/events:batch` enforces a configurable 1 MiB default JSON-body limit for both declared and streamed request lengths after authentication and CSRF checks. Validated OIDC issuer and subject resolve to one enabled PostgreSQL membership; missing, disabled, ambiguous, or changed membership fails closed, invalidates the active browser session on its next tenant API request, and leaves tenant claims non-authoritative. The adapter activates only when an OIDC client registration exists; no production provider or credentials are configured. `/apps/worker` is a same-version non-web process that consumes tenant-tagged SQS messages, permanently rejects missing or mismatched tenant tags, and deletes a valid message only after its persisted event is visible and its tenant-scoped score refresh succeeds. Valid messages that cannot be completed are bounded by an SQS redrive policy and retain their tenant tag in `accepted-events-dlq`. Both call `/libs/rules-scoring` for the rules engine. `/apps/console` is an Angular onboarding and risk shell whose current-risk and account-history routes follow backend cursors through explicit Load more interactions using the same-origin opaque server session. Both views provide loading, empty, safe error, retry, populated, pagination-loading, and pagination-completion states. The shell now posts logout through Angular's framework-managed XSRF support, exposes pending/success/safe-error states, and returns the signed-out user to onboarding. The console has no client tenant switch and never reads or stores API keys, bearer/OIDC tokens, tenant headers, or the HttpOnly session cookie. Pagination cursors remain only in in-memory view state. The retired scaffold is gone, and no active module has MongoDB or Redis. Local Compose starts PostgreSQL and LocalStack SQS/S3.
...
- Branch: `cursor/pr-041r-account-upsert-9d98`
...
- Database migrations: `V1__platform_bootstrap.sql`, `V2__tenant_scoped_events.sql`, `V3__account_scores.sql`, `V4__accepted_event_outbox.sql`, `V5__browser_sessions.sql`, `V6__tenant_memberships.sql`, `V7__account_score_history.sql`, `V8__tenant_accounts.sql`
...
| Area | State |
| --- | --- |
| Product docs | ADR-001, ADR-002, PRD, data classification, ADR template, threat model, account-upsert, events:batch, and cursor-paginated current/history prediction-read OpenAPI exist |
| Backend | `platform-service` with Actuator, JDBC, Flyway, PostgreSQL Spring Session, browser CSRF, POST logout, request-time active-membership revocation, conditional provider-neutral OIDC login, enabled-membership resolution, API-key/session TenantContext, tenant-scoped account upsert, byte-bounded tenant-scoped event ingestion, event persistence, transactional outbox delivery to SQS, shared `RULES_BASELINE` current/history scores, and cursor-paginated prediction reads; `worker` retains valid messages until their persisted event can be scored and permanently rejects invalid tenant tags |
| Tests | platform-service context, health, PostgreSQL bootstrap, browser-session cookie/persistence/CSRF/absolute-expiry/logout/lifecycle-revocation, OIDC Authorization Code/state/nonce/PKCE contract, OIDC membership mapping and failure, membership resolution and deny-by-default routes, tenant-isolation, account-upsert create/retry/concurrency/malformed/cross-tenant behavior, declared/streamed event-body limits, event-batch, persistence, transactional-outbox enqueue, rules-score, prediction-read, prediction-cursor, and immutable tenant-history tests; shared rules-scoring unit tests; worker context-load, consume, bounded DLQ redrive, successful-delete, and rescore tests; console onboarding/current-risk/history contract tests |
| Persistence | Flyway V1 bootstrap, V2 `ingested_events` / `ingest_receipts`, V3 `account_scores`, V4 `accepted_event_outbox`, V5 Spring Session tables, V6 tenants/users/memberships, V7 append-only `account_score_history`, and V8 tenant-owned `accounts`; worker uses the same PostgreSQL store without owning Flyway |
...
| Angular console | onboarding, current-risk, and account-history routes; both risk views use explicit cursor-backed Load more controls with loading, completion, and safe retry states, the same-origin session, and semantic tables; the shell provides CSRF-protected logout with pending, success, and safe failure feedback; local `ng serve` proxies only API/authentication paths to `platform-service`; no login UI yet; API keys and OAuth tokens are forbidden from browser storage by ADR-002 |

## What PR-041R added

- Flyway V8 creates tenant-owned accounts with a database-enforced composite `(tenant_id, account_external_id)` key and bounded, non-blank identifiers
- `POST /v1/accounts:upsert` derives tenant authority only from verified `TenantContext`, returns 201 for the single create effect, and returns 200 for same-tenant retries
- One PostgreSQL `INSERT ... ON CONFLICT DO NOTHING` makes concurrent retries deterministic without Redis, an in-memory lock, or another infrastructure dependency
- Closed-payload validation rejects missing, malformed, oversized, and additional fields with Problem Details 400; missing credentials fail before any database effect
- Integration tests prove exact response shape, retries, eight concurrent attempts, same-ID cross-tenant isolation, forged tenant rejection, malformed input, and no unauthenticated effect

## What PR-040R added

- The blueprint-defined `POST /v1/accounts:upsert` path and a minimal request/response containing only the explicit customer-supplied `account_external_id`
- `account_external_id` is required, 1–128 characters, and is the domain idempotency key; first creation returns 201 and later retries in the same verified tenant return 200 with one account effect
- The same external ID may exist independently across tenants, while the request accepts no tenant field or arbitrary customer metadata
- Contract guardrails fail if the endpoint, schemas, response semantics, retry invariant, or no-client-tenant rule disappears
- No Java, persistence, migration, console, authentication, dependency, or infrastructure change

## What PR-039R added

- A configurable `platform.events.max-request-size` with a 1 MiB default, scoped only to `POST /v1/events:batch`
- Authenticated and CSRF-valid event requests are read at most `limit + 1` bytes before JSON parsing, so both declared and unknown-length/streamed oversized bodies fail with tenant-safe Problem Details 413
- Exact-limit bodies reach the controller unchanged, while unrelated endpoints remain outside the filter
- OpenAPI and threat-model memory now reflect the enforced byte boundary; event-count validation remains 500
- No rate limiter, decompression support, persistence, tenant authority, dependency, migration, queue, or infrastructure change

## What PR-038R added

- An accessible shell Sign out control with duplicate-submit protection, progress feedback, a generic retryable error, and signed-out confirmation
- Same-origin `POST /logout` through Angular `HttpClient`; Angular's built-in XSRF support owns the `XSRF-TOKEN` to `X-XSRF-TOKEN` transfer, while application code never reads cookies or stores credentials
- Successful logout returns the console to `/onboarding`; local `ng serve` now proxies the exact `/logout` path to `platform-service`
- Console contracts cover the request boundary, safe outcomes, navigation, accessibility states, proxy allowlist, and existing no-browser-credential guardrails
- No backend, IdP end-session, provider selection, tenant authority, dependency, migration, or infrastructure change

## What PR-037R added

- Browser-authenticated `/v1/**` requests re-resolve the session's validated issuer and subject and require the exact internal user and tenant membership to remain enabled
- Disabling the user, tenant, or membership now fails closed on the next tenant API request, invalidates the PostgreSQL-backed session, clears the production cookie, and returns generic Problem Details 401 without redirect
- Valid machine API-key requests remain independent from browser membership lifecycle state and do not cause a stale ambient browser cookie to become tenant authority
- Request-time validation intentionally performs one PostgreSQL membership lookup per authenticated browser API request; caching and background bulk revocation remain deferred
- No Angular, IdP, audit-sink, migration, dependency, tenant-authority, or infrastructure change

## What PR-036R added

- A POST-only `/logout` handled by Spring Security; GET cannot log a browser out and unsafe requests require the existing `XSRF-TOKEN` / `X-XSRF-TOKEN` contract
- Successful logout invalidates the PostgreSQL-backed session, clears the production `__Host-tm_session` cookie, issues a fresh readable CSRF cookie, and returns `204 No Content` without an HTML redirect
- The machine API-key CSRF exemption is now limited to `/v1/**`, so an `X-Api-Key` header cannot bypass logout CSRF protection
- Integration coverage proves rejected logout attempts retain the session and successful logout deletes its JDBC row; no Angular, IdP, tenant-authority, migration, or infrastructure change

## What PR-035R added

- A configurable `platform.security.session.absolute-lifetime` with a secure eight-hour default, separate from the existing 30-minute idle timeout
- A pre-authentication filter that derives the deadline only from the server-side session creation time and invalidates the PostgreSQL session at the boundary
- Browser `/v1/**` requests receive tenant-safe Problem Details 401 after expiry without an HTML redirect
- Clock-controlled integration coverage proves recent activity cannot extend the deadline and valid machine API keys remain unaffected by an expired browser session
- No frontend, migration, tenant-authority, OIDC-provider, or infrastructure change
...
- Local `ng serve` proxies `/v1`, `/oauth2`, and `/login` to `platform-service` at `http://localhost:8080`, preserving one browser origin as required by ADR-002
- The proxy contract permits only those three path prefixes and keeps the original host; it adds no credentials, tenant authority, catch-all route, CORS policy, or production infrastructure
- Console contract tests fail if the proxy wiring is missing or broadened unexpectedly
- Production routing, IdP selection, login UI, backend security, TLS automation, and infrastructure remain out of scope
...
- The current-risk client accepts an optional opaque cursor, encodes it into later `/v1/predictions?limit=50` requests, and keeps it only in in-memory view state
- An accessible Load more control appends later pages, suppresses duplicate account rows, disables concurrent loads, and reports loading, completion, and safe retry states
- Later-page failures preserve already rendered predictions and the retry cursor; a 401 receives a session-safe message and other failures hide backend details
- Account-history pagination, filtering, sorting, infinite scrolling, backend changes, and infrastructure remain out of scope

## What PR-030R added

- Each account in the current-risk table links to `/risk/:accountExternalId/history`
- The history view requests the first 50 revisions from the existing API with same-origin credentials, an encoded account path, runtime response validation, and no browser tenant authority or authentication secrets
- Loading, empty, safe 401/generic error, retry, and accessible newest-first table states cover the first customer-visible history slice
- History pagination controls, login UI, score explanations, retention automation, and backend changes remain out of scope
...
- History pagination controls, retention deletion, learned models, explanations, and infrastructure remain out of scope
