...
| T-04 | Event ingestion / Denial | API availability | Abusive or buggy client | Oversized batch, nested JSON, or decompression bomb (resource exhaustion) | OpenAPI/JSON Schema limits, batch size, payload and decompression limits, plan rate limits | Tests for oversized body, malformed input, and bounded batch | Engineering | Open until payload, decompression, and rate limits exist; prediction list is now bounded to 500 |
