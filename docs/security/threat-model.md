...
| T-07 | Queues / Tampering | Tenant-tagged messages | Compromised worker or mis-published message | Process Tenant B work on a Tenant A consumer | Tenant key on every message; worker uses workload identity; no shared static secret | Consumer rejects missing/mismatched tenant tag | Engineering | Open until the worker consumes and rejects mismatched tenant tags |
